...
const appointments = [
  ...
  { id:'4', serviceName:'Invalid', clientName:'Riley', clientEmail:'r@e.com', staffMemberName:'Dr. A', timezone:'UTC', startsAt:'2026-03-14T15:00:00.000Z', endsAt:'2026-03-14T14:00:00.000Z', status:'scheduled' }
];
...
test('returns empty set for invalid date windows', () => {
  assert.deepEqual(filterAppointments(appointments, { from:'not-a-date', to:'2026-03-20T00:00:00.000Z', staff:'all', status:'all' }), []);
  assert.deepEqual(filterAppointments(appointments, { from:'2026-03-20T00:00:00.000Z', to:'2026-03-14T00:00:00.000Z', staff:'all', status:'all' }), []);
});

test('excludes invalid appointment records with malformed durations', () => {
  const out = filterAppointments(appointments, { from:'2026-03-14T00:00:00.000Z', to:'2026-03-16T00:00:00.000Z', staff:'all', status:'all' });
  assert.deepEqual(out.map((apt) => apt.id), ['1', '2', '3']);
});
